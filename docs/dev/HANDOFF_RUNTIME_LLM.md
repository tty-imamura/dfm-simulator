...
**スキーマ追補(2026-07-29・第40便 40B — beta 先行、任意キー。以下いずれも SYSTEM_PROMPT には
未開放 = LLM生成には公開しない。validatePreset が受理してクランプ・警告するだけ)** —
- `abBody`(台帳4-77): 粒子への patch を含む「おすすめA/B」のワンタップレシピ
  `{label:{ja,en}, targets:[粒子index...], patch:{spin/m/vx/vy}}`。UI 表示専用データで、
  物理エンジンには影響しない(A/B開始ボタンが読むだけ)。
- `claims`(4-78 パイロット): 説明文中の数値主張を機械可読にした
  `[{id, metric, expected:{min,max}, control?:{patch,expected}, testId}]`。表示はしない
  (データ層のみ・UI表示は次便裁定)。今回は agnjet/cosmicweb の2プリセットのみに限定したパイロット実装。
- `validT`(第39便 39E・台帳4-75 E2の追記漏れの解消): 「評価済み最大時間」の HUD 表示専用メタ
  (正の有限数のみ受理・物理には一切使わない)。
