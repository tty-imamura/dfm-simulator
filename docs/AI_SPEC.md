...
- **背景複素決定力の宣言(第276便a・原仮定者の裁定〔第66報〕(1))**: **`physics.backgroundComplex`** ——
  裁定の字義は「**『背景決定力 D₀』と別途『背景複素決定力』を用意する。『引きずり減衰 q』の算出や
  計算など影響箇所を精査する。両者はサンプルによって変動するがシチュエーションで決まるので
  事前予測が可能。**」である。**`physics.D0` / `physics.D0pull` とは別の鍵**で、**別の量**を持つ。
  - **なぜ別か(単位)**: スカラーの D₀ は「距離に反比例する決定力の総和」で **[M/L]**。
    複素決定力場(空間メッシュ・`need:"mesh"`)の重みは w=m/(r²+ε²) なので、**背景の重み W₀ は [M/L²]**、
    その**分子 A₀ = Σ w_i u_i は [M/(L·T)]・向きつき**である。**同じ数を D₀ と W₀ の両方に入れない**し、
    **W₀ と A₀ を同じ数にもしない**(反対向きに動く源は分子で相殺しても重みには正で残る)。
  - **正準形**: `{background, W0, A0, gradW, gradA, dWdt, dAdt, refPos?, note?}`。
    `background` の受理値は **`"solar-excluded"` / `"heliocentric"` / `"galactic"` /
  ...
  - **`"zero"` は「ゼロと宣言」・未宣言は「未確定」**である(**未入力の背景を勝手に静止ゼロと扱わない**)。
    `"zero"` は **W₀ も他の成分もすべて 0** でなければ拒否し、**W₀=0 のときに A₀ や微分が 0 でない**宣言も拒否する。
  - **エンジンのどの経路からも読まれない**(力学・光学・帳簿に 1 バイトも接続しない)。
    接続は次便以降に**宣言鍵で切り替える**。**未宣言は `physics` に入らない**ので
    **presetSig・エクスポート JSON が 1 文字も変わらない**(宣言すると署名は変わる ——
    **「ゼロと宣言」と「未宣言」も別の署名**である)。
  - **内蔵 131 本はこの鍵を 1 本も宣言していない**(本便が足したのは受理契約だけで、
    **D₀・D0pull の値は 1 本も変えていない**)。
  ...
  - **`core.axisMode` / `core.azimuthDeg` / `core.precessionRate`(第275便e・opt-in・既定 off —— **SYSTEM_PROMPT には載せていない**)**:
    ...
    **面内の方位角**と**歳差角速度**を宣言する口を足した(第227便のジャイロ分岐 φ̇=殻スピンは従来どおり)。
    ...
    - **これは 3D の実装ではない。** エンジンの力学が読むのは従来どおり **z 射影 `J_z` だけ**で、
      ...
      **正射影**した線分(共通設定「コア軸の投影線を表示(宣言した粒子だけ)」・表示専用)。
      **既定は「宣言した粒子だけ ON」**(第276便c で言葉を統一 —— トグルは ON だが、
      宣言の無い粒子には何も描かないので内蔵の画面は 1 ピクセルも変わらない)。
...
## 9. 説明の 4 つ目の節 `descStruct.brief`(第276便f — **表示専用の宣言**)

第276便f(第66報(5)②「『要約』が長くなりがちなので数行以下の概要を用意する」)で、構造化説明
`descStruct` に**任意の 4 つ目の節 `brief`(概要)**を足した。これは **`summary`/`observe`/`control` と
同格の鍵ではない** —— 「要約の要約」であって、`description` の一部ではない。

- **純分割の契約は 1 バイトも変わっていない。** 検証器 `validatePreset` の連結一致は従来どおり
  `summary + observe + control === description` で、**`brief` はこの検査に入らない**。
  したがって `brief` を宣言しても `description` は 1 文字も変わらない。
- **`presetSig` にも保存 JSON の物理にも入らない**(`presetSig` は `description` も `descStruct` も
  直列化しない)。**エンジンは 1 箇所も読まない。**
- **上限は 120 字**(`HP.DESC_BRIEF_CAP`)。超えた分は**警告つきで切り詰める**
  (`descStruct.brief が上限120字を超過 — 切り詰めます`)。**`descStruct` ごとは落とさない** ——
  概要が長すぎても本文の純分割は壊れていないためである(`summary` 等の節上限 `DESC_SECT_CAPS`
  超過は従来どおり純分割が壊れて `descStruct` ごと削除される)。文字列でない `brief` は警告つきで無視する。
- **宣言が無いときは `summary` から機械抽出する**(`HP.descBriefExtract` —— **純関数・決定的**。
  実行時状態も乱数も読まない)。規則は ①空白を 1 個へ畳む ②全文が上限以内ならそのまま返す
  (「…」も付けない)③超えていれば文末記号(`。．.！？!?` +閉じ括弧)で文に割り、1 文目を採り、
  **足しても上限を超えないときだけ** 2 文目を足す ④上限 −1 字で切る。**空白のある言語では
  単語の途中で切らない**(上限の 3/4 より後ろの空白まで戻す)⑤末尾に「…」。
- 表示は**言語追随**(`HP.descBriefOf` は `LANG` に従い、en は `en.descStruct.brief` を読む)。
  出所は `HP.descBriefSource` が `"declared"` / `"extracted"` / `"none"` で返す。
- **`SYSTEM_PROMPT` には載せない**(AI 生成には開放していない —— 受理して検証するだけである。
  §5 の逐語ブロックは 1 バイトも変わっていない)。生成 AI が `brief` を書いてきた場合は、
  上の上限と型検査を通して**表示にだけ**使われる。
- 内蔵 131 本の内訳(第276便f 時点): **ja は宣言 2・抽出 129**、**en は宣言 48・抽出 83**。
...
## 12. 第279便c の宣言鍵(原仮定者の裁定〔第69報〕・統括の読み R62/R63・**SYSTEM_PROMPT には載せない**)
...
### 12.1 `physics.backgroundComplex.sources` —— 源の分割(明示天体と背景の排他)
...
### 12.2 `physics.backgroundComplex.frame` —— 凍結参照系
...
### 12.3 `physics.meshVelocity` —— 複素決定力による座標変換(opt-in 外部ステップ)
...
- **拒否**: `kFrame>0`(**既定 kFrame=1 のまま宣言しても拒否** —— `physics.kFrame:0` を明示する)・`geoPN=3`(**第280便c: `spaceMesh.lawVersion:"vMinusU"` の輸送経路としてだけ受ける —— §13**)・`spaceMesh` の宣言(同じく vMinusU の契約だけ例外)・
...
- **内蔵の宣言**: **0 本**(第280便c: geoPN=3・vMinusU の principle の診断コピー 🔁🌒 が**輸送経路として**宣言する —— §13)。
- QA: **`preset.meshVelocity`**・**`docs.bgCompose`**・**`docs.bgbudget2-sync`**・**`docs.d0sites-sync`** ⑤(背景鍵の読み口は宣言した外部ステップの準備だけ)。

## 13. 第280便c の宣言鍵 —— **geoPN=3 の契約**(原仮定者の裁定〔第70報〕「近点移動の差分を精査する — geoPN=3 として進める」・統括の読み R65・**SYSTEM_PROMPT には載せない**)
