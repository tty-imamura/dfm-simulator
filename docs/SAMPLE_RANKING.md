...
- 版: v6f(第258便c・2026-09-12 — 第50報 UI 7 件: **サンプルの宣言は 1 文字も変えていない**。変えたのは表示だけで、(a) 分類「現実較正」40 本を **DFM 版 29 / kF0 版 11** の 2 チップへ分けた(規則は既定 `physics.kFrame===0` の**派生値** —— プリセットにも保存 JSON にも書かない)、(b) 🌞solarInner と 🌇venusReal を **単独ファミリーの代表**にした(family "mercury" は ☄️🪨 の 2 本に。`presetSig` は familyId を見ないので署名不変)、(c)「ベースのスケール」の選択肢を 2 行にし、☀️球状星団(e16/e12/e35)と 🌌実単位銀河(e19/e15/e38)を**廃止**した(使っていた 🍇🫐🌃🛞 の `scaleExp` は**書き換えていない** —— 観測転写では指数が実単位の宣言だから)。**ランクも力学も動いていない**〔全内蔵 120 本 × 600 步ビット同一・署名差 0 本〕)。
