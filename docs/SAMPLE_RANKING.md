...
- 版: v6f(第258便b・2026-09-12 — 第50報「空間メッシュの実装について精査する」: **銀河の場の宣言を 3 本にそろえた**(署名便)。🌌 galaxy・🎡 galaxyStd に `overlays.galaxyField:{unSource:"disk",unFit:"affine"}` を、🎠 galaxyMeshSpiral に `slipThreshold:0.9` を宣言。表示と記録が読む場が**上位 2 体の連星場から銀河の局所場へ**変わる(空間メッシュ表示を出したときだけ効く)。**表示と記録だけの変更**で、**全内蔵 120 本 × 600 步の状態はビット同一**・ランクも力学も動いていない。他の未宣言 117 本には一括追加していない)。
