...
## `solutions.json`(公表解の台帳 —— 第271便b で新設・第278便a で暦の解 2 件を登録)

観測 CSV の `solution_id` 欄が指す**公表解の台帳**である(1 件 = 1 つの公表解: 論文・表・モデル・元期・時系)。
`solution_id` は「その行の値がどの公表解のどの列から来たか」を機械可読にする**出所の鍵**であって、印でも σ でも判定でもない。

- 綴りは 2 形: タイミング解 `<著者><西暦4桁>-<モデル>`(例 `Meng2025-DDFWHE`)と、**暦の解**
  `<3 文字の暦記号><3 桁の番号>-<西暦4桁>`(例 `PLU060-2024` —— 暦名だけでは版を区別できないので公表論文の西暦を添える)。
- 登録は 6 件: `Kramer2021-DDS` / `Fonseca2014-DDGR` / `Singha2026-DDH` / `Meng2025-DDFWHE` / **`PLU060-2024`**
  (Brozović & Jacobson 2024 AJ 167 256・Table 8 / Table 10)/ **`PLU043-2015`**(Brozović et al. 2015 Icarus 246 317)。
- 欄を埋めるのは、note が語境界で `solution=<id>` を持つ行だけ(読取器 `tests/lib-w270b-obscsv.mjs` の `solutionTag()`)。
  台帳に無い解タグの行は**空欄のまま**(空欄は「解が無い」ではなく「台帳に登録していない」)。
- 台帳の各 id は、少なくとも 1 行の CSV 行から指されていなければならない(孤立 id を置かない)。
  機械固定は QA `lint.solutionId`(⑦ 参照切れの検査)。**署名(html)は動かさない。**
