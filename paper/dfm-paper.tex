% =====================================================================
% Determinacy-Field Mechanics -- arXiv draft
% CURRENT VERSION: v0.12 (2026-08-22, wave 167).
% VERSION OF RECORD: v0.9 (2026-07-20) = repository tag paper-v1
% (release v1.27.0 series), Zenodo v1 doi:10.5281/zenodo.21454189.
% The present manuscript is a POST-RECORD EDITORIAL REVISION of that
% record: no number, claim, equation or window has moved since v0.9 --
% only notation canonicalization (K_t written as kappa), the title and
% subtitle, and typesetting/citation work.  Re-depositing the revision
% on Zenodo is a submission-time action (TODO(release) below).
% The single version statement for this manuscript is the two lines
% above -- they are repeated in paper/README.md and nowhere else.  The
% list below is the revision history.
% Written per docs/dev/HANDOFF_PAPER_V2.md (sole authoritative writing brief).
...
%       (Historical note: the "Input canon" line above reflects v0.2;
%       current canon is PHYSICS v1.9 / THEORY_SYNTHESIS v1.7+ /
%       index.html v1.27 — see paper/README.md.)
% v0.6: title change approved (roadmap decision 4-19, 2026-07-20):
%       "Emergent Inverse-Square Gravity" -> "Inverse-Square Gravity"
%       (C2 states a decomposition, not an emergence from nothing) and
%       "Spin-as-Heat Thermodynamics" -> "Spin-as-Heat Analogies"
%       (consistent with negative claim 6). No other change.
% v0.7: release-readiness sync (seventh external AI review of the
%       repository, 2026-07-20): implementation version references
%       updated to 1.27 (frame-time accounting fix + prompt/E12 key
%       sync + viewport accessibility -- per-step physics unchanged),
%       CI suite count updated to 109 (two release-readiness gates
%       added). No claim, number, or equation changed.
% v0.8: standard-source citations added (sixth-review Major 12):
%       CODATA 2022 (RMP 97, 025002), IAU 2015 Resolution B3
%       (Prsa et al., AJ 152, 41), and Ashby 2003 (Living Rev.
%       Relativity 6, 1) cited from Table I (GPS design value) and
%       Table IV (benchmark inputs). All three verified against
%       primary records (publisher DOI / arXiv) on 2026-07-20.
% v0.9: version of record finalized — submission tag paper-v1
%       (= v0.8 merge, release v1.27.0 series) and its Zenodo DOI
%       10.5281/zenodo.21454189 inserted; TODO(submission) resolved;
%       sentence moved to present tense. No other change.
% v0.9 (wave 143, 2026-08-19): second external review round, typesetting
%       and citation only -- Eq. (15) broken with split and three tables
%       set as paragraph columns (zero major overfull boxes), the
%       abstract's thermodynamic-analogy limitation sharpened by one
%       sentence, the 2D pseudo-3D force
%       (broken Gauss law) given a fuller paragraph, and the paper's
%       target tag separated from the current app build in
%       Reproducibility. No claim, number, or equation changed, so the
%       version of record stays v0.9.
% v0.10 (wave 156, 2026-08-21): third external review round.  Metadata
%       and one figure-caption cleanup only.  The panel labels of Fig. 5
%       were converted in the SVG from the legacy K_t form to kappa, so
%       the caption's bridging sentence is gone; affiliation, version and
%       date brought in line with Papers 2-3; the post-record status of
%       this revision stated in Reproducibility.  No claim, number,
%       equation or figure datum changed, so the VERSION OF RECORD stays
%       v0.9.
% v0.11 (wave 161, 2026-08-22): fourth external review round.  Wording
%       only -- no number, equation, figure datum, window or claim
%       moved, so the VERSION OF RECORD stays v0.9.
%       (a) P1-1.  The Abstract's E12 sentence now carries the same
%           limiting clause the body already states (Sec. IV D): the
%           option is calibrated in the static, single-dominant-source,
%           test-particle domain, and its per-source form omits the
%           multi-source cross terms.
%       (b) P1-2.  Sec. I B now separates three levels of claim that a
%           reader can otherwise run together -- an implementation
%           reproducing a reference value, a model extension calibrated
%           on a stated domain, and a physical mechanism independently
%           verified -- and says which level E12 occupies (the second).
%           The motivation for introducing E12 at all (the honest
%           failure of the base dynamics on Mercury) is stated in one
%           more sentence in Sec. IV D.
%       (c) M-4.  The Abstract states in one sentence why the model
%           posits the 1/d^2 force on a plane, and points at the
%           existing broken-Gauss-law discussion in the body.
% v0.12 (wave 167, 2026-08-22): title shortened.  The main title is now
%       "Determinacy-Field Mechanics: An Executable Machian Toy
%       Universe"; everything the old single-sentence title carried --
%       tunable frame dragging, inverse-square gravity, spin-as-heat
%       analogies, and the three weak-field benchmarks read as
%       consequences of one calibration and not as evidence -- is
%       relocated to the subtitle, so no element of it is dropped.  This
%       is a typesetting/metadata change: no number, equation, figure
%       datum, window or claim moved, so the VERSION OF RECORD stays
%       v0.9 and this remains a post-record editorial revision.  The
%       Reproducibility paragraph that enumerates what differs from the
%       deposited version is synchronized in the same wave.
% TODO(release): re-deposit this post-record revision on Zenodo (a new
%       version under concept DOI 10.5281/zenodo.21454189) at submission
%       time, together with the shortened title and its subtitle.
...
\usepackage{hyperref}
\hypersetup{
  pdftitle={Determinacy-Field Mechanics: An Executable Machian Toy
    Universe. Tunable Frame Dragging, Inverse-Square Gravity, and
    Spin-as-Heat Analogies; the GPS Clock Offset, Solar-Limb
    Deflection and Shapiro Delay as Consequences of One Calibration,
    Not as Evidence},
...
% wave 153: the spacetime coefficient is written in the canonical form
% kappa (parameter kappaT) used by the implementation and docs/PHYSICS.md
% since wave 128.  kappa = 1/K_t, so the correspondence mapping reads
% kappa = G/c^2 and psi = W*kappa (a multiplication).  The macro name is
% kept so that the diff stays confined to the definition and to the
% formulas that had to be inverted.
\newcommand{\Kt}{\kappa}
\newcommand{\Dz}{D_0}
...
% free of the line breaks in the title (hyperref warnings), as in paper 3.
% wave 167: the main title is now a short noun phrase naming the object
% ("an executable Machian toy universe").  Nothing the old one-sentence
% title carried is dropped -- the mechanism list and the three weak-field
% benchmarks, together with the "one calibration, not evidence" reading,
% move to the subtitle line, set smaller so that the title block does not
% grow.  Metadata/typesetting only; no claim or number is affected.
\title{\texorpdfstring{Determinacy-Field Mechanics: An Executable
Machian Toy Universe\\
{\normalsize Tunable Frame Dragging, Inverse-Square Gravity, and
Spin-as-Heat Analogies; the GPS Clock Offset, Solar-Limb Deflection
and Shapiro Delay as Consequences of One Calibration, Not as
Evidence}}%
{Determinacy-Field Mechanics: An Executable Machian Toy Universe. Tunable Frame Dragging, Inverse-Square Gravity, and Spin-as-Heat Analogies; the GPS Clock Offset, Solar-Limb Deflection and Shapiro Delay as Consequences of One Calibration, Not as Evidence}}

\author{Tetsuya Imamura}
% Affiliation confirmed 2026-07-18 (author's decision).
\affiliation{Independent Researcher, Tokyo, Japan}

\date{July 20, 2026 (v0.9, version of record); revised August 21, 2026
  (v0.10); revised August 22, 2026 (v0.11 and v0.12)}
...
Under one calibration of that field, $\Kt = G/c^2$, the model's
weak-field clock-and-light sector reproduces the leading benchmark
formulas for the GPS clock offset ($+38.5\ \mu$s/day), the solar-limb
light deflection ($1.7512''$) and the Shapiro delay
($\approx 281\ \mu$s)---agreement that is a consequence of that single
calibration, not evidence for the model.
...
% wave 161 (M-4): why the plane carries a 1/d^2 force at all.  The
% statement is the one already made in the body (Sec. II B and
% contribution C2); nothing new is claimed here.
The plane is idealized in one respect that should be said at the
outset: the field is \emph{posited} to fall off as $w \propto m/d$, so
that the force is inverse-square as in three dimensions, rather than
the $1/d$ that a Gauss-law-consistent field on a plane would give.
The choice keeps Kepler orbits and $1/d^2$ forces in the picture a
student sees, at the price of the flux reading of the law and of the
shell theorem, and the body states that price where the axioms
introduce it.
...
The observational inputs behind each of those three benchmarks are
specified in an appendix, and the agreement is at the level of the
leading weak-field formulas, with the
...
% wave 161 (P1-1): the Abstract now carries the same limiting clause the
% body already states in Sec. IV D, in the same vocabulary.
That recovery is stated with its domain rather than without one: E12 is
calibrated in the \emph{static}, \emph{single-dominant-source},
\emph{test-particle} regime, and the per-source form actually
implemented omits the multi-source cross terms of the total-potential
expression, which the two forms share only when one source dominates.
...
theory.
The last limitation is the sharpest: the single signed spin variable
reproduces the \emph{bookkeeping} of the zeroth and first laws on a
stated domain, but it carries no microstate count, no entropy defined
from one, and no second law, so none of its ``temperatures'' may be
read as thermodynamic temperatures.
...
dimensionless potential $\psi = W\Kt$ sets both the local clock
rate and the optical refractive index---the coefficient $\Kt$ (the
implementation parameter \texttt{kappaT}) carries the dimensions of
$G/c^2$, length per mass, and is \emph{not} Einstein's gravitational
constant $\kappa = 8\pi G/c^4$ of general relativity, with which it
shares only the letter; and spin doubles as heat, generating
...
109-item continuous-integration suite).
...
% wave 161 (P1-2): the three levels of claim are separated explicitly,
% so that E12 cannot be read at the third level.  No claim is added or
% withdrawn; this states the scope the paper already operates under.
Three levels of claim run through this paper, and English does not
separate them by itself, so we separate them before the list:
(i)~\emph{an implementation reproduces a reference value}---the code,
run as described, returns the number an established formula returns;
(ii)~\emph{a model extension is calibrated on a stated domain}---a rule
added to the model agrees with observation inside a domain declared
along with it, the agreement being a consequence of the calibration
rather than independent support for it; and
(iii)~\emph{a physical mechanism is independently verified}---the
model's mechanism is shown to be the mechanism operating in nature.
No claim in this paper is of the third kind.
The three benchmark agreements of Sec.~\ref{sec:corr} are of the first
and second kinds, which is what C3 and the abstract mean by
``a consequence of that single calibration, not evidence for the
model''; and the geodesic option E12 is of the second kind, not the
third.
E12 is a labeled extension whose 1PN agreement follows from the single
calibration $\Kt = G/c^2$ within the static, single-dominant-source,
test-particle domain stated in Sec.~\ref{sec:e12}; recovering
$42.98''$/century there is not evidence that the model's
clock-and-light sector is how gravity works.
...
Under the parameter mapping $\Kt = G/c^2$, the model's proper-time and
...
sections.
It is worth being explicit about what that suspension costs, because
the cost is structural rather than cosmetic.
With $w \propto m/d$ on a plane there is no two-dimensional Poisson
equation the field satisfies: $\nabla^2 W$ does not reduce to a
multiple of the mass density, the flux of $\nabla W$ through a closed
curve is not proportional to the mass it encloses, and consequently
no shell theorem holds---the field inside a uniform ring of matter
does not cancel the way a Newtonian 3D shell does.
Every statement in this paper that would ordinarily be justified by
Gauss's law is therefore justified instead by direct summation over
sources, which is what the implementation does and what the
verification checks measure (V9 compares the summed acceleration with
the numerical gradient of $W$ to $2.4\times10^{-4}$).
The practical consequence for the reader is that extended-body
intuition imported from three dimensions is not reliable here: rings,
disks, and shells must be integrated, not argued about, and the
model's outer-rotation results in Sec.~\ref{sec:exp3} are quoted as
measured band averages for exactly that reason.
...
\begin{equation}
\Kt = \frac{G}{c^2} \approx 7.43\times10^{-28}\ {\rm m\,kg^{-1}},
\label{eq:kappa}
\end{equation}
...
Each benchmark requires, besides $\Kt = G/c^2$, a set of fixed
...
mapping $\Kt = G/c^2$, with the observational inputs of each row fixed
...
Observable & DFM ($\Kt = G/c^2$) & Experimental constraint & Verdict \\
\hline
GPS net clock-rate difference (satellite $-$ ground) &
$+38.5\ \mu$s/day (gravity $+45.7$, motion $-7.2$; inputs in
Table~\ref{tab:inputs}) &
$\approx +38.6\ \mu$s/day (GPS system design
value~\cite{Ashby2003}) &
...
% wave 161 (P1-2): the motivation for E12 -- the honest failure that
% produced it -- stated before the derivation, in one short paragraph.
The option exists because of a failure, and the failure is worth
stating before the construction.
The base dynamics E1$'$--E11 does \emph{not} reproduce the static 1PN
perihelion advance of Mercury; that is negative claim~5
(Sec.~\ref{sec:negative}), and it is reported as a result rather than
repaired quietly.
Instead of absorbing the discrepancy into a fitted coefficient, we
asked a narrower question: whether the line element the model already
uses for clocks and light, taken seriously as a matter action, supplies
the missing term by itself.
It does, in the static sector, and the domain in which it does is
declared with it below.
...
% wave 143 (P0-3): broken with split so that the line fits the column
% (it previously overran the column by 71 pt).  Content unchanged.
\begin{equation}
\begin{split}
...
$GM_\odot/(c^2a) \approx 2.7\times10^{-8}$ is not resolvable in the
default single-precision state arrays, the
...
a double-precision engine option later removes this restriction and
reproduces the advance at Mercury's real parameter directly (end of
this subsection).
...
$\Kt = G/c^2$ with no additional free coefficient.
...
\emph{Direct reproduction at Mercury's real invariant.}
A released engine revision lifts the single-precision restriction with
three opt-in, default-off switches that leave every other run
bit-identical: a double-precision state option
(\texttt{stateCarry}: accelerations, positions and velocities in
double precision with compensated velocity kicks), an explicit
1PN-source declaration (\texttt{pnSource}, replacing the fixed mass
threshold whose deflection-angle form scales as $c^2$), and
tier-real constants $G_{\rm sim} = 6.674$, $c_0 = 3\times10^4$ (the
real-value display conventions of the stellar tier, with
$\Kt = G/c_0^2$ under the same single calibration).
A test particle with Mercury's real invariant
$GM/(c_0^2a) = 2.55\times10^{-8}$ and $e = 0.206$ is then integrated
directly; the perihelion drift is measured as the least-squares slope
of the orbit-averaged Runge--Lenz angle over 600 orbits, subtracting
the $\lambda_{\rm PN} = 0$ baseline (which isolates the softening
precession $-3\pi(\varepsilon/a)^2$; the same policy as V18).
The result is $\Delta\varpi = 5.029\times10^{-7}$\,rad/orbit against
the analytic $5.020\times10^{-7}$ (ratio $1.002$), i.e.\
$0.1037''$/orbit $\times\,414.93$ orbits/century $= 43.0''$/century,
within $0.2\%$ of the observed $42.98''$/century; an invariant ladder
at $5\times10^{-7}$, $10^{-7}$ and $2.55\times10^{-8}$ stays at ratio
$1.001$--$1.002$ throughout (harness \texttt{tests/exp-obscal.mjs};
run records in \texttt{tests/out/obscal-results.json}).
The 1PN advance at Mercury's real parameter is thus recovered
dynamically, not only by the formula-level mapping above.
...
(version 1.27; the per-step physics engine is unchanged since the
E12/E8R revisions of versions 1.20--1.21) on the same engine; each figure is
...
(open problem L17).}
\begin{ruledtabular}
% wave 143 (P0-3): all three columns made paragraph columns so that the
% long check labels wrap inside the column instead of overrunning it.
\begin{tabular}{p{0.36\columnwidth}p{0.20\columnwidth}p{0.24\columnwidth}}
Check & Domain & Result \\
\hline
...
mass (preset \texttt{lensing}, $\Kt = 1/150$): mean bending angle
...
The machine gate V8 (run at $\Kt = 1/500$ for coefficient sensitivity)
...
(a)~Ray fan in preset \texttt{lensing} ($\Kt{=}1/150$, 26 rays,
...
$\Kt{=}1/500$): the upper and lower halves bend unequally because the
medium is advected by the rotating frame.
The panel labels are written in the canonical form $\Kt$.
...
agreement at $\Kt = G/c^2$ (Table~\ref{tab:calib})---the model is
...
same $\Kt = G/c^2$ with no additional free coefficient (checks
...
$\Dz \sim 10^{27}\,{\rm kg\,m^{-1}} \approx 1/\Kt$, whereas rotation
...
The version of record for this paper is fixed by the annotated
repository tag \texttt{paper-v1} (release v1.27.0 series), archived at
Zenodo as
\href{https://doi.org/10.5281/zenodo.21454189}
{doi:10.5281/zenodo.21454189}, so that every number in the paper is
tied to one immutable commit.
The present text is a post-record editorial revision of that record: it
changes no number and no claim, and differs from the deposited version
only in notation canonicalization, the shortened title with its
subtitle, and typesetting; the revision is re-deposited at submission
time.
That tag, and not the head of the repository, is what this paper
describes: development has continued past it, and the simulator a
reader downloads today is a later build (v1.41.0, since promoted to the
release channel) whose presets, engine options, and gate counts
have moved on.
Numbers quoted here should therefore be reproduced against
\texttt{paper-v1}; the later builds are a different, separately
documented artifact.
The simulator source code is also deposited on Zenodo as a software
record---an archive of the code, not a DOI of this manuscript---whose
concept DOI
\href{https://doi.org/10.5281/zenodo.21454188}
{doi:10.5281/zenodo.21454188} resolves to all versions, the current
release v1.41.0 being archived as
\href{https://doi.org/10.5281/zenodo.22025091}
{doi:10.5281/zenodo.22025091}.
Machine-verification outputs are part of that record: the
continuous-integration suite (\texttt{tests/qa.mjs}, 109 items
...
$\Kt = G/c^2$ & $\approx 7.43\times10^{-28}\ {\rm m\,kg^{-1}}$ &
...
A physical intuition for why the calibration constant is an
\emph{inverse line density} (m\,kg$^{-1}$), i.e.\ a gravitational
length per unit mass: the determinacy $W = \sum m/d$ itself has
...
$\Kt = G/c^2$, each number in that table is recomputable with no
further choices.
...
The values are standard reference values: fundamental constants
consistent with CODATA~\cite{CODATA2022}, solar and planetary
quantities consistent with the IAU 2015 nominal
values~\cite{Prsa2016}, and the GPS orbit, velocity, and clock-offset
inputs from the standard GPS relativity literature~\cite{Ashby2003}.}
\begin{ruledtabular}
% wave 143 (P0-3): paragraph columns so the long symbolic inputs wrap.
\begin{tabular}{p{0.28\columnwidth}p{0.30\columnwidth}p{0.20\columnwidth}}
Input & Value & Used in \\
\hline
...
\begin{ruledtabular}
% wave 143 (P0-3): narrower configuration column (0.72 -> 0.64) so the
% widest row no longer overruns the column.
\begin{tabular}{p{0.16\columnwidth}p{0.64\columnwidth}}
Run & Configuration \\
\hline
