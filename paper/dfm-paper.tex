...
by default):
\begin{equation}
\vect{a}_{\rm 1PN} = \frac{\lambda_{\rm PN}}{c_0^2}
 \left[\Bigl((\alpha-\tfrac12)v^2 - (1+2\alpha)U\Bigr)\nabla U
 - (1+2\alpha)(\nabla U\cdot\vect{v})\,\vect{v}\right],
\label{eq:e12}
\end{equation}
with $U = \sum_j Gm_j/\sqrt{d_j^2+\varepsilon^2}$ (so $\nabla U$ is
the E4 acceleration).
...
$GM_\odot/(c^2a) \approx 2.7\times10^{-8}$ is unmeasurable in a
single-precision engine, the machine checks keep Mercury's
...
The claim domain of this first stage is deliberately narrow: sources
are massive bodies only; velocities are referred to the static
background ($\vect{u} = 0$); the E6$'$ transport is disabled for
matter under \texttt{geoPN} $= 1$ (the action already contains
