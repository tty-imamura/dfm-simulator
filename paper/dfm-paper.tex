% =====================================================================
% Determinacy-Field Mechanics -- arXiv draft (v0.2, 2026-07-19)
...
of the outer rotation velocities relative to the $k_F = 0$ control---as
a mechanism demonstration inside the model; with $k_F = 0$ the dynamics
reduces exactly to Newtonian $N$-body gravity.
Under the single calibration $\Kt = c^2/G$, with the observational
inputs of each benchmark fixed in an appendix, the model's weak-field
clock and light sector reproduces the leading weak-field benchmark
formulas for the GPS clock offset, the solar-limb light deflection, and
...
With dragging amplified to $k_F = 1$, disk systems raise their outer
rotation velocities relative to the $k_F = 0$ Keplerian control run
from identical initial conditions (machine-gated gain
$0.055 \pm 0.007$; outer-band ratio $1.082$ at 6000 steps).
This demonstrates \emph{mechanism sufficiency for outer-velocity
enhancement inside the model}---not flatness of the curve
...
Every weak-field coefficient of the clock--light sector below,
including the effective lensing coefficient $(a+b)/\Kt = 2/\Kt$, is a
consequence of this single axiomatic choice, not an output forced by
the existence of a single potential $\psi$ alone.
...
\label{eq:E6pair}
\end{align}
where $n_{ij}$ is the residual pair torque and the spin transfer is at
equal angular acceleration; the background torque
$[\vect{r}_i \times \phi_{\rm bg}\Delta\vect{p}_i]_z$ goes to the
reservoir ledger.
All increments are computed from the same-step frame field and applied
...
(A7$'$), one potential and one choice, not by a second calibrated
coefficient.

Table~\ref{tab:calib} confronts this sector with the standard
weak-field benchmarks---successes and failure in the same table.
...
$u_\varphi(r)$ and the outer stars ride the rotating frame, raising the
outer $v(r)$; with $k_F = 0$ the curve declines in Keplerian fashion.
...
to Keplerian---not a demonstrated flat segment.
...
A12$'$: spin inferred from radiated color reproduces the true spin
within $5\%$.
...
claims are made.
We also note (single-size restriction, stated reason): with unequal
particle sizes E10$'$ uniformizes the angular velocity $s$, not the
...
particle sizes (E10$'$ uniformizes angular velocity, not $T = Is^2$,
...
The version of record for this paper is fixed by a git tag on the
...
JSON records described below; the exact configurations (all physics
...
and generates pressure; single particle size only & A9, A10,
