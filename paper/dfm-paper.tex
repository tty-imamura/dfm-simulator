...
59-item continuous-integration suite).
...
As $\Dz$ is scanned from large to small, the interior frame passes from
``absolute'' (unaffected by the shell) to ``relational'' (co-rotating
with the shell): the entrainment angular rate of interior test
particles rises from $0$ toward the shell rate.
...
\begin{figure}[t]
\centering
\framebox[0.95\columnwidth]{\rule{0pt}{4.2cm}\ Fig.~1 placeholder}
\caption{\label{fig:mach}%
Entrainment angular velocity of interior test particles versus
background determinacy $\Dz$ (log scale), normalized to the shell
rotation rate.
Generated from preset \texttt{mach} with a manual scan
$\Dz \in \{0.05, 0.5, 2, 8, 32\}$, all other parameters at preset
defaults; measured via \texttt{HP.sim} direct drive.}
\end{figure}
...
\begin{figure}[t]
\centering
\framebox[0.95\columnwidth]{\rule{0pt}{4.2cm}\ Fig.~2 placeholder}
...
\begin{figure}[t]
\centering
\framebox[0.95\columnwidth]{\rule{0pt}{4.2cm}\ Fig.~3 placeholder}
\caption{\label{fig:drag}%
Periapsis drift per orbit versus center spin sign:
regression for prograde spin, advance for retrograde, control at
$k_F{=}0$.
Generated from verification hook V6 ($a{=}60$, $e{=}0.5$,
$S{=}\pm0.05$, $k_F\in\{0,1\}$, $\Dz{=}0.05$, dissipation off;
periapsis detection with $r$-gate and minimum spacing); illustrative
trail from preset \texttt{drag}.}
\end{figure}
...
\begin{figure}[t]
\centering
\framebox[0.95\columnwidth]{\rule{0pt}{4.2cm}\ Fig.~4 placeholder}
\caption{\label{fig:clocks}%
Clock rates $\tau/t$ versus analytic predictions for kinematic (V12),
combined (V13), and twin (V16) configurations; all deviations
$<10^{-3}$.
Generated from verification hooks V12/V13/V16 at their embedded
parameters (V12: $\Kt{=}10^6$, $v/c_0\in\{0,0.3,0.6\}$; V16:
$v{=}0.5c_0$); illustrative display from preset \texttt{twin}.}
\end{figure}
...
\begin{figure}[t]
\centering
\framebox[0.95\columnwidth]{\rule{0pt}{4.2cm}\ Fig.~5 placeholder}
\caption{\label{fig:light}%
Ray-fan trajectories past a spinning center for $s_c = \pm0.5$,
showing asymmetric bending and captured circulating rays (an optical
analogue of a photon sphere).
Generated from presets \texttt{lensing} ($\Kt{=}150$, 26-ray fan,
$x_0{=}-300$, $|y_0|\le255$, $\ud l{=}2.73$) and \texttt{spinlens};
quantitative gate: verification hook V8 (asymmetry
$6.77\times10^{-2}$~rad at $\Kt{=}500$).}
\end{figure}
...
outward (mean core radius $34.5 \to 108.8$, a $3.2\times$ expansion)
---pressure emerging from A10 without a pressure law being written in.
...
\begin{figure}[t]
\centering
\framebox[0.95\columnwidth]{\rule{0pt}{4.2cm}\ Fig.~6 placeholder}
\caption{\label{fig:heat}%
(a)~Left/right mean temperature time series equilibrating (preset
\texttt{gas}, single particle size).
(b)~Hot-core expansion against a cold shell (preset \texttt{pressure};
mean core radius $34.5 \to 108.8$).
Both generated from the named presets at default parameters via
\texttt{HP.sim} with the built-in temperature overlay.}
\end{figure}
...
preset IDs and verification hooks stated in each caption.
