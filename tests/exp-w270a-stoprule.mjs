...
import { stopRuleFor, stopDecision, machineIndependenceProbe, STOP_RULE_VERSION,
  CLASS_MAX_STEPS, PRESET_MAX_STEPS, PRESET_NEED_PERIASTRA,
  WALL_CEILING_SEC_DEFAULT } from './lib-w270a-stoprule.mjs';
...
for (const p of (cal.presets || [])) {
  ...
  for (const sr of list) {
    ...
    const tol = (sr.boundBy === 'declared-max-steps') ? 0 : Math.ceil((sr.orbMax || 60) / 2) + 1;
    try {
  ...
  replay: { nStages: rows.length, compared: compared.length,
    ...
    finding: (compared.length && differing.length === 0)
      ? '**基点 f6c19b4 の ' + compared.length + ' 段すべてで、步数も近点数も同じである** —— '
        ...
        + '**測っている中身は変えていない**)。'
      : '**' + differing.length + ' 段で基点と違う**(differingRows に並べた)。隠さずに読むこと。' },
...
console.log('  ② 逆算照合: 基点と同じ段 ' + identical.length + ' / ' + compared.length
  + '(違う段 ' + differing.length + ' / 新しい段 ' + newStages.length + ')');
