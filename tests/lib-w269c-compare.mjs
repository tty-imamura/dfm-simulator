...
export function loadObsCsv(fp) {
  ...
  for (const line of lines) {
    ...
    if (head === null) {
      head = parseCsvLine(line).map((z) => String(z).trim());
      if (head.indexOf('body') < 0)
        throw new Error('CSV の 1 行目がヘッダでない(body 欄が無い): ' + fp);
      continue;
    }
    const c = parseCsvLine(line);
    const at = (name) => { const i = head.indexOf(name); return (i < 0) ? null : (c[i] === undefined ? null : c[i]); };
    const str = (name) => { const z = at(name); return (z === null || String(z).trim() === '') ? null : String(z); };
    rows.push({ body: str('body'), quantity: str('quantity'),
      value: finiteNumber(at('value')), unit: str('unit'),
      source: str('source'), url: str('url'), retrieved: str('retrieved'),
      note: str('note') || '',
      sigmaCol: finiteNumber(at('sigma')),
      // 第270便b が足す予定の欄(**無ければ null** —— 列位置ではなく名前で引く)
    ...
    Object.defineProperty(rows[rows.length - 1], 'header', { value: head, enumerable: false });
  }
  return rows;
}
