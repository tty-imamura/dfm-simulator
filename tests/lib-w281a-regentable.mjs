...
import { scopeHash, stableJsonSha } from './lib-w281a-scope.mjs';

export const REGEN_TABLE_VERSION = 'w281a-regentable-1';

const S = (key, cmd, outs, sec, o) => Object.assign({ key, cmd, outs, sec, secSource: 'w280-chain',
  alwaysRun: false, role: 'current', after: [], env: {} }, o || {});
...
export function planRegen(o) {
  ...
  const scopeOn = (hAbs, sc) => {
    const k = hAbs + '|' + JSON.stringify(sc);
    if (!scopeCache.has(k)) { let r = null; try { r = scopeHash(hAbs, sc); } catch { r = null; } scopeCache.set(k, r && r.scopeComplete ? r.scopeSha256 : null); }
    ...
    for (const out of st.outs) {
      ...
      if (!t || !m.targetSha256) why.push(out + ': 対象の刻印が無い');
        ...
        if (m.targetSha256 !== hS) {
          ...
          } else why.push(out + ': 対象 html の hash も領域 hash も一致しない' + (m.scope ? '' : '(領域の宣言なし)'));
        }
      } else if (shaFile(abs(t)) !== m.targetSha256) why.push(out + ': 対象 ' + t + ' の hash が違う');
      // コード
      if (!Array.isArray(m.code) || !m.code.length) why.push(out + ': 器・lib の刻印が無い');
      else for (const c of m.code) if (c.sha256 && shaFile(abs(c.file)) !== c.sha256) why.push(out + ': コード ' + c.file + ' が変わった');
      // 入力
      for (const inp of (m.inputs || [])) {
        ...
        if (st2 && stableJsonSha(abs(inp.file)) === st2.stableSha256) continue;
      ...
      for (const st2 of (m.inputsStable || [])) {
        ...
        if (stableJsonSha(abs(st2.file)) !== st2.stableSha256) why.push(out + ': 入力 ' + st2.file + ' の中身(安定 hash)が変わった');
      }
    }
  ...
  while (changed) {
    ...
    for (const row of rows.values()) {
      ...
      if (hit.length) { row.status = 'recheck'; row.reasons.push('依存先が走る: ' + hit.join(', ')); changed = true; }
    }
  }
...
export default { REGEN_TABLE_VERSION, REGEN_STEPS, stepsByOut, alwaysRunOuts, historyOuts, planRegen };
