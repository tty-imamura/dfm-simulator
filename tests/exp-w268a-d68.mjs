...
const convergenceProposal = {
  ...
  caveats: [
    ...
    '**ok になっても合否は言わない** —— 判定は門(tests/exp-w249b-calaudit.mjs)の仕事である。'
      + '**第270便a(AD8)で 📡 は THREE_STAGE_REGISTRY に登録され**、換算後の判定が正式な門へ'
      + '繋がった(**登録は合格の宣言ではない** —— 門がどちらを出すかは走らせて測る)',
  ],
...
const out = {
  ...
  verdict: '**本器は合否を言わない**(3 段の収束が宣言の条件を満たしても、判定は門'
    + '(tests/exp-w249b-calaudit.mjs)の仕事である)。第270便a(AD8)で 📡 は '
    + 'THREE_STAGE_REGISTRY に登録され、換算後の判定が門へ繋がった —— **門が何を出したかは'
    + '門の JSON を読むこと**(ここに書き写さない)。',
