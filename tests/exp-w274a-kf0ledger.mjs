// 第274便a(第64報・kF0 棚卸し便)「**kFrame=1 が成立しないサンプルで kF0 版が成立しているか**」の
// 棚卸し表を、**正本 JSON から機械生成する**器である。
//
// ■ 何をするか
...
//
// 実行: node tests/exp-w274a-kf0ledger.mjs
// 出力: tests/out/kf0ledger-w274a.json(正本)・docs/KF0_LEDGER_v1.45.md(機械生成の表)
...
const OUT_JSON = path.join(ROOT, 'tests', 'out', 'kf0ledger-w274a.json');
const OUT_DOC = path.join(ROOT, 'docs', 'KF0_LEDGER_v1.45.md');

export const LEDGER_VERSION = 'w274a-1';
...
export const LABEL_RULE = '**kF1 側の観測成立が ✗(否 3σ)**かつ **kF0 側の 4 列がすべて ✓** の行だけに付く。'
...
function sideAssess(rec, k, health) {
  ...
  if (!qs.length) {
    return { present: false, run: COL.NA, structure: COL.NA, numeric: COL.NA, observation: COL.NA,
  ...
  return { present: true, run, structure: COL.NA, numeric, observation,
    runEvidence,
...
for (const p of KF_PAIRS) {
  ...
  const row = { k0: p.k0, k1: p.k1, why: p.why, emoji0: a.emoji, emoji1: b.emoji,
    ...
    note: fSame ? '**f 固定・k のみの対照**である'
      : '**k のみの対照ではない** —— 質量補正 f が一緒に動いている'
        + '(f ' + fx(a.massFactor, 8) + ' → ' + fx(b.massFactor, 8) + ')。'
        + '**独立な質量補正 f は別軸であり、f と k は別試験で振る**。' };
  pairs.push(row);
  a.pair = { with: p.k1, side: 'kF0', kOnly: fSame, note: row.note };
  b.pair = { with: p.k0, side: 'kF1', kOnly: fSame, note: row.note };
  // **f が同じ対**は「f 固定・k のみの対照」を持つ行として数える(同一プリセットの診断コピーより
  // 弱い対照である —— 別サンプルなので初期条件・停止条件の宣言も別である。`how` で区別する)。
  if (fSame) for (const [x, y] of [[a, p.k1], [b, p.k0]]) {
    if ((x.kOnlyContrast || {}).has) continue;
    x.kOnlyContrast = { has: true, how: 'paired-preset', pairedWith: y,
      why: '対 `' + y + '` と **f が同じ**(' + fx(x.massFactor, 8) + ')で kFrame だけが違う。'
        + '**別サンプルの対**なので、同一プリセットの診断コピーより弱い対照である'
        + '(初期条件・停止条件の宣言も別である)。' };
  }
}
...
// ---- ❄️ カロン C0(kF1)/ C1(kF0)
{
  ...
  const mk = (tag) => {
    ...
    const observation = Number.isFinite(nSig) ? (Math.abs(nSig) <= 3 ? COL.OK : COL.NG) : COL.NA;
...
for (const r of (nslock.rows || [])) {
  ...
  const mk = (tag) => {
    const z = v[tag]; if (!z) return { present: false, run: COL.NA, structure: COL.NA,
      numeric: COL.NA, observation: COL.NA, why: '変種 ' + tag + ' が無い' };
    ...
    return { present: true,
      ...
      numeric: COL.NA,
      observation: Number.isFinite(z.nSigmaExt) ? (Math.abs(z.nSigmaExt) <= 3 ? COL.OK : COL.NG) : COL.NA,
      ...
      observationEvidence: { omegaDotExt: num(z.omegaDotExt), ratioExt: num(z.ratioExt),
...
for (const r of rows) {
  ...
  const kf0AllOk = [a && null, b.run, b.structure, b.numeric, b.observation]
    .slice(1).every((z) => z === COL.OK);
...
// ================================================================ 出力(JSON)
const out = {
  meta: Object.assign(provenanceMeta({ root: ROOT, wave: '第274便a(第64報・kF0 棚卸し便)',
    target: 'beta/index.html', inputs: INPUTS, code: CODE }),
  { ledgerVersion: LEDGER_VERSION,
    question: '**「kFrame=1 が成立しないサンプルで、kF0 版が成立しているか」**(原仮定者の優先課題・第64報)',
    answerRule: '**4 列がすべて ✓ のときだけ「成立」と書く**。完走だけを成立と書かない。'
  ...
  columns: COLUMNS, label: { text: LABEL, rule: LABEL_RULE },
  summary, pairs, rows,
...
const lines = [];
lines.push('# kF0 棚卸し表(v1.45 開発中 — 第274便a)');
lines.push('');
lines.push('**この文書は機械生成である**(生成器 `tests/exp-w274a-kf0ledger.mjs` / 正本'
  + ' `tests/out/kf0ledger-w274a.json`)。**手で数字を書き換えない** —— 生成器を直して作り直す。');
lines.push('');
lines.push('答える問い: ' + out.meta.question);
lines.push('');
lines.push('## 0. 読み方(4 列 — 統括の検証項目 R28)');
lines.push('');
lines.push('| 列 | 何を見るか |');
lines.push('| --- | --- |');
...
lines.push('');
lines.push('列は 4 列を **走行 構造 数値 観測** の順に並べたものである。');
lines.push('');
lines.push('| # | 系 | 宣言 kFrame | f | kF1 側(走行 構造 数値 観測) | kF0 側(走行 構造 数値 観測) | f 固定・k のみの対照 | ラベル |');
lines.push('| ---: | --- | ---: | ---: | --- | --- | --- | --- |');
calRows.forEach((r, i) => {
...
lines.push('');
lines.push('| # | 系列 | kF1 側(走行 構造 数値 観測) | kF0 側(走行 構造 数値 観測) | kF0 側の根拠 | ラベル |');
lines.push('| ---: | --- | --- | --- | --- | --- |');
...
console.error('[w274a] kF0 棚卸し表: 行 ' + rows.length + '(較正 ' + calRows.length
  + ' / 診断 ' + diagRows.length + ')・kF0 配布 ' + kf0Applied.length
  + ' 行・ラベル ' + labelled.length + ' 行・「kF1 否・kF0 合」 ' + kf1NgKf0Ok.length + ' 行');
console.error('[w274a] wrote ' + OUT_JSON);
console.error('[w274a] wrote ' + OUT_DOC);
