...
//   (5) 数える対象は **QA の対象 HTML の `S._core`** である。QA `lint.coreBudget` は
//       **beta と root(旧世代 index.html)の両方で走る**(どちらにも `S._core` があり、
//       機能の有無で分かれる UI ブロックと違って SKIP する理由が無い。SKIP は
//       `S._core` そのものが見つからないときだけである)。
//       **第260便d の訂正**: この行は第259便d まで「数えるのは beta だけ」と書いてあり、
//       **QA が root も走らせている事実と食い違っていた**(数え方の宣言と実装のずれ)。
//       正本は QA の側なので、宣言をそちらへ合わせた。参考値は root 34802 字である。
...
export const CORE_REFERENCE = {
  ...
  '第259便d(boxHist を外へ)': 35197,
  '参考: root(旧世代 index.html)': 34802,   // 第260便d: QA は root でも走る(数え方 (5))
};
