...
- **内蔵シミュレーション**(「1サンプル=1効果」の5グループ構成 — v1.31 で「銀河」を新設。
  総数は入れ替わるため記載しない — 現物はアプリのプリセット選択が正。v1.9 全面刷新、
  以後追加・整理を継続):
  ...
  - **銀河**(v1.31 新設): 🌌銀河の回転曲線(kFrame=1 で外縁速度が増強。実測vsケプラーの
    グラフ付き) /
    🕶️ダークローターの銀河(高速スピンのコア+圧倒的重量の外殻「ダークローター」が決定
    フレーム u_φ を押し上げ外縁回転を増強。ハロー20体はレール駆動=規定運動 — 外部要素
    バッジに表示) /
